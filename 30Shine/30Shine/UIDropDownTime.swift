...
enum UIDropDownTimeAnimationType : Int {
    case Default
    case Bouncing
}

@objc protocol UIDropDownTimeDelegate {
    func dropDownTime(dropDown: UIDropDownTime, didSelectOption option: String, atIndex index: Int)
    optional func dropDownTableWillAppear(dropDown: UIDropDownTime)
    optional func dropDownTableDidAppear(dropDown: UIDropDownTime)
    optional func dropDownTableWillDisappear(dropDown: UIDropDownTime)
    optional func dropDownTableDidDisappear(dropDown: UIDropDownTime)
}

class UIDropDownTime: UIControl, UITableViewDataSource, UITableViewDelegate {
    ...
    var options = [String]()
    var delegate: UIDropDownTimeDelegate!
    var animationType: UIDropDownTimeAnimationType = .Default
    ...
    func setup() {
        ...
        self.addTarget(self, action: #selector(UIDropDownTime.touch), forControlEvents: .TouchUpInside)
    }
    ...
    func showTable() {
        ...
        case .Default:
            ...
            UIView.animateWithDuration(0.9, delay: 0, usingSpringWithDamping: 0.5, initialSpringVelocity: 0.1, options: .TransitionFlipFromTop, animations: { () -> Void in
                ...
                self.table.frame = CGRect(x: CGRectGetMinX(self.frame), y: CGRectGetMaxY(self.frame)+5, width: CGRectGetWidth(self.frame), height: 150)
                self.table.alpha = 1
    ...
    func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        delegate.dropDownTime(self, didSelectOption: "\(options[indexPath.row])", atIndex: indexPath.row)
